  ...
  <div style="display: flex">
    <div
      ...
      style="
        ...
        overflow-y: auto;
      "
      v-if="dataPercentage.length > 0"
    >
      <div v-for="(item, index) in dataPercentage" :key="index">
        <p
          v-if="index + 1 === 1"
          style="font-weight: bold; animation: index1 0.5s ease-in-out infinite"
        >
          <img
            :src="item.warehouseImage"
          ...
          <img
            ...
            alt=""
          />
          {{ item.shelfName }} => {{ item.location }} ({{ item.code }}) :
          {{ getTwoDecimals(item.percentage) }}%
        </p>
      ...
      <div class="form-section">
        ...
        <div class="form-group">
          ...
          <select
            ...
            @change="SearchFloor('old')"
          >
            <option
              v-for="(item, index) in currentFloorData"
              :key="index"
              :value="item"
            >
              {{ item.name }}
          ...
          <div class="dropdown-container">
            ...
            <div class="ward-dropdown">
              <div class="ward-grid" v-if="quantityLocation !== null">
                <div
                  v-for="(cell, cellIndex) in quantityLocation.quantity"
                  :key="cellIndex"
                >
                  <div
                    v-if="
                    ...
                    <div
                      v-if="
                        checkQuantityLocationProduct(
                          quantityLocation.productLocationAreas,
                          cell
                        ) > 0 &&
                        checkQuantityLocationProduct(
                          quantityLocation.productLocationAreas,
                        ...
                        @click="
                          (event) =>
                            openFrame(
                              ...
                              cell,
                              quantityLocation.productLocationAreas,
                              quantityLocation.productPlans,
                              'old',
                              'swapBgOld_' + quantityLocation.id + '_' + cell
                            )
                        "
                        style="background-color: blueviolet; font-size: 12px"
                      >
                        {{
                          quantityLocation.productLocationAreas.some(
                            (x) => x.location == cell
                          ) ||
                          quantityLocation.productPlans.some(
                            (x) => x.location == cell
                          )
                            ? cell +
                              " - " +
                              quantityLocation.name +
                              " (Có sản phẩm)"
                            : cell + " - " + quantityLocation.name
                        }}
                      </button>
                    </div>
                    <div v-else>
      ...
      <div class="form-section" v-if="isKhoMoiVisible">
        ...
        <div class="form-group">
          ...
          <select
            ...
            @change="SearchFloor('new')"
          >
            <option
              v-for="(item, index) in currentFloorDataNew"
              :key="index"
              :value="item"
            >
              {{ item.name }}
          ...
          <div class="dropdown-container">
            ...
            <div class="ward-dropdown">
              <div class="ward-grid" v-if="quantityLocationNew !== null">
                <div
                  v-for="(cell, cellIndex) in quantityLocationNew.quantity"
                  :key="cellIndex"
                >
                  <div
                    v-if="
                  ...
                  >
                    ...
                    >
                      ...
                      {{
                        checkLocationPlan(
                          quantityLocationNew.productInPlans,
                          quantityLocationNew.id,
                          cell
                        ).title
                      }}
                      :
                      {{
                        checkLocationPlan(
                          ...
                          quantityLocationNew.id,
                          cell
                        ).shelfsOld == quantityLocationNew.id &&
                        checkLocationPlan(
                          quantityLocationNew.productInPlans,
                          quantityLocationNew.id,
                          cell
                    ...
                    <div
                      v-if="
                        checkQuantityLocationProduct(
                          quantityLocationNew.productLocationAreas,
                          cell
                        ) > 0 &&
                        checkQuantityLocationProduct(
                          quantityLocationNew.productLocationAreas,
                      ...
                      <button
                        ...
                        @click="
                          openFrame(
                            ...
                            cell,
                            quantityLocationNew.productLocationAreas,
                            quantityLocationNew.productPlans,
                            'new',
                            'swapBgNew_' + quantityLocation.id + '_' + cell
                          )
                        "
                        style="background-color: blueviolet; font-size: 12px"
                      >
                        {{
                          quantityLocationNew.productLocationAreas.some(
                            (x) => x.location == cell
                          ) ||
                          quantityLocationNew.productPlans.some(
                            (x) => x.location == cell
                          )
                            ? cell +
                              " - " +
                              quantityLocationNew.name +
                              " (Có sản phẩm)"
                            : cell + " - " + quantityLocationNew.name
                        }}
                      </button>
                    </div>
                    <div v-else>
        ...
        >
          ...
          >
            ...
            <img
              :src="item?.image"
              alt="Image"
              :class="'frame-image' + ' ' + 'images_' + item.id_product"
            />
            <div style="display: flex; width: 150px; flex-wrap: wrap">
              <div
                v-for="(imageItem, indexImage) in item.images"
                :key="indexImage"
                style="margin: 0 10px"
              >
                <img
                  :src="imageItem"
                  width="50px"
                  @click="swapImage('images_' + item.id_product, imageItem)"
                  alt=""
                />
              </div>
            </div>
...
const findOnePlan = async (id) => {
  ...
  if (res.data.success) {
    ...
    planNew.value.locationOld = res.data.content.localtionOld;
...
const shelfNew = ref([]);
const shelfCurrentNew = ref(null);
...
const loadDataPercentage = async () => {
  const res = await axios.get(
    hostName + "/api/Deliverynote/GetWarehouseSalesPercentage",
    getToken()
  );
  if (res.data.success) {
...
const updateData = async () => {
  ...
  if (res.data.content) {
    ...
    document.querySelector("." + classNameData.value).style.backgroundColor =
      "yellow";
  } else {
...
const updateDataLocationNew = async () => {
  ...
  if (classNameNew.value != null && classNameNew.value != "")
    document.querySelector("." + classNameNew.value).style.backgroundColor =
      "transparent";

  document.querySelector("." + classNameDataNew.value).style.backgroundColor =
    "yellow";
  closeFrame();
  Toast.success("Success !!!");
};
...
const findOneArea = async (type) => {
  ...
  document.body.style.overflow = "hidden";
  if(type === 'old'){
    if (shelfCurrentOld.value != null && shelfCurrentOld.value.id != null && shelfCurrentOld.value.id != undefined) {
    const res = await axios.get(
      hostName +
        `/api/Product/FindOneByArea?id=${shelfCurrentOld.value.id}`,
      getToken()
    );
    if (res.data.success) {
    ...
    if (shelfCurrentNew.value != null && shelfCurrentNew.value.id != null && shelfCurrentNew.value.id != undefined) {
    const res = await axios.get(
      hostName +
        `/api/Product/FindOneByArea?id=${shelfCurrentNew.value.id}`,
      getToken()
    );
    if (res.data.success) {
...
const findAllShelf = async (type) => {
  if(type === 'old'){
    if(DataOneArea.value != null && DataOneArea.value.id != null && DataOneArea.value.id != null){
      const res = await axios.get(
      hostName +
        `/api/Shelf/FindByArea?id=${DataOneArea.value.id}&page=1&pageSize=2000`,
      getToken()
    );
    if (res.data.success) {
    ...
    const res = await axios.get(
      hostName +
        `/api/Shelf/FindByArea?id=${DataOneAreaNew.value.id}&page=1&pageSize=2000`,
      getToken()
    );
    ...
    if (res.data.success) {
      if (res.data.content.data.length > 0) {
        ...
        shelfNew.value = [];
          shelfCurrentNew.value = null;
          quantityLocationNew.value = null;
      }
    }
  }
};
const findAllArea = async (type) => {
  if(type === 'old'){
    if(currentFloor.value != null && currentFloor.value.id != null && currentFloor.value.id !== undefined){
      const res = await axios.get(
      hostName +
        `/api/Area/FindOneByFloor?id=${currentFloor.value.id}`,
      getToken()
    );
    if (res.data.success) {
      if (res.data.content.data.length > 0) {
    ...
    if(currentFloorNew.value != null && currentFloorNew.value.id != null && currentFloorNew.value.id != undefined){
      const res = await axios.get(
      hostName +
        `/api/Area/FindOneByFloor?id=${currentFloorNew.value.id}`,
      getToken()
    );
    if (res.data.success) {
      if (res.data.content.data.length > 0) {
...
const SearchWarehourse = async (type) => {
  const res = await axios.get(
    hostName +
      `/api/Floor/FindByWareHouser?id=${
        type === "old"
          ? currentWarehouse.value.id
          : currentWarehouseNew.value.id
...
const loadDataWarehouse = async (type) => {
  ...
  loadDataFloor(
    type === "old" ? currentWarehouse.value : currentWarehouseNew.value,
    type
  );
  Toast.success("Success");
};
...
const loadDataFloor = async (id, type) => {
  if(type === 'old'){
    if(id != null && id.id != null && id.id != undefined){
      const res = await axios.get(
